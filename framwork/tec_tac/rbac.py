...
Extension manifests declare permission groups. Tec-Tac 1.0.x persists generic
role grants in the existing ExtensionRolePermission compatibility table.
Extensions should import this module rather than the compatibility model.
"""
...
def permission_groups(plugin_id: str) -> dict[str, tuple[str, ...]]:
    for plugin in get_plugins():
        if plugin.plugin_type == "extension" and plugin.plugin_id == plugin_id:
...
def has_extension_permission(user, codename: str) -> bool:
    ...
    return Permission.objects.filter(role_id=role.id, codename=codename, granted=True).exists()

def set_extension_permission(role, codename: str, granted: bool):
    _validate_codename(codename)
    if not isinstance(role, Role): raise TypeError("role must be an accounts.models.Role instance")
    Permission = _permission_model()
    row, _ = Permission.objects.update_or_create(role_id=role.id, codename=codename, defaults={"granted": bool(granted)})
    return row

def grant_extension_permission(role, codename: str): return set_extension_permission(role, codename, True)
def revoke_extension_permission(role, codename: str): return set_extension_permission(role, codename, False)
...
def get_role_permissions(role, plugin_id: str) -> dict[str, bool]:
    ...
    Permission = _permission_model()
    stored = {row.codename: row.granted for row in Permission.objects.filter(role_id=role.id, codename__in=codenames)}
