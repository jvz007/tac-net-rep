...
SUPPORTED_KEYS = frozenset({"id", "type", "version", "python_paths", "django_apps"})
...
def _load_manifest(plugin_type: str, plugin_dir: Path) -> PluginSpec | None:
    ...
    if unknown:
        raise RegistryError(
            f"Plugin manifest contains unsupported keys {unknown!r}: {manifest_path}"
        )
...
def _discover_root(plugin_type: str, root: Path) -> list[PluginSpec]:
    ...
    for child in sorted(root.iterdir(), key=lambda p: p.name):
        ...
        if spec.plugin_id in seen_ids:
            raise RegistryError(
                f"Duplicate {plugin_type} ID discovered: {spec.plugin_id!r}."
            )
...
def _validate_pairs(
    extensions: Iterable[PluginSpec], reportsets: Iterable[PluginSpec]
) -> None:
    ...
    if orphan_reportsets:
        raise RegistryError(
            "Reportset(s) without matching extension: " + ", ".join(orphan_reportsets)
        )

    missing_reportsets = sorted(extension_ids - reportset_ids)
    if missing_reportsets:
        raise RegistryError(
            "Extension(s) without matching reportset: " + ", ".join(missing_reportsets)
        )


def discover_plugins(
    extensions_root: Path | None = None, reportsets_root: Path | None = None
) -> tuple[PluginSpec, ...]:
    """Discover and validate convention-based extension/reportset pairs."""
...
def legacy_plugins() -> tuple[PluginSpec, ...]:
    ...
    # No manifest means this folder is ignored by convention discovery and only
    # loaded through this compatibility entry.
    return (
        PluginSpec(
...
def get_plugin(plugin_id: str, plugin_type: str | None = None) -> PluginSpec:
    """Resolve one registered plugin by ID, optionally constrained by type."""
    matches = [
        plugin
        for plugin in get_plugins()
        if plugin.plugin_id == plugin_id
        and (plugin_type is None or plugin.plugin_type == plugin_type)
    ]
    if not matches:
    ...
    if len(matches) > 1:
        raise RegistryError(
            f"Plugin ID {plugin_id!r} matches multiple plugin types; specify a type."
        )
    return matches[0]
