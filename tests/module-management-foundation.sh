...
grep -q '"manage_modules": can_manage_privileged_operations(user)' "${ROOT}/framwork/tec_tac/views.py" || fail "privileged module capability mapping missing"
grep -q 'CORE_PRIVILEGED_PERMISSION = "core.privileged_operations"' "${ROOT}/framwork/tec_tac/rbac.py" || fail "Core privileged permission missing"
