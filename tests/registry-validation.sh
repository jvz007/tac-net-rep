...
printf '[TEST] Checking Tec-Tac manifest validation and optional reportset ownership.\n'
...
with tempfile.TemporaryDirectory() as td:
    ...
    plugins = discover_plugins(ext, rep)
    assert [(p.plugin_type, p.plugin_id) for p in plugins] == [("extension", "demo")]

    # A reportset may not exist without its owning extension.
    (ext / "demo" / "tec_tac.json").unlink()
    (rep / "demo" / "tec_tac.json").write_text(json.dumps({
        "id": "demo", "type": "reportset", "version": "0.1.0",
        "python_paths": ["."], "django_apps": []
    }))
    try:
        ...
        raise AssertionError("orphan reportset was not rejected")
