...
REQUIRED_FILES=(
    ...
    "${SOURCE_FRAMEWORK_DIR}/tec_tac/resource_views.py"
...
VERIFY_RESOURCE_CODE="from tec_tac.capabilities import get_capability,capability_status; from tec_tac.resources import resource_contract_metadata; p=get_capability('core.resources',version='>=1.0.0,<2.0.0'); m=resource_contract_metadata(); assert m['version']=='1.0.0' and set(m['resource_types'])=={'client','site','agent'}, m; assert all(hasattr(p,n) for n in ('list_clients','get_client','list_sites','get_site','list_agents','get_agent','resolve_resource')); status=capability_status('core.resources'); assert status['available'] and status['capability_version']=='1.0.0', status; print('TEC-TAC Resource Directory OK:', m['resource_types'].keys())"
...
mkdir -p "${SERVER_MAINTENANCE_ROOT}/jobs" "${SERVER_MAINTENANCE_ROOT}/cancel-requests" "${SERVER_MAINTENANCE_ROOT}/logs" "${SERVER_MAINTENANCE_ROOT}/running" "${SERVER_MAINTENANCE_REGISTRY_ROOT}" "${SERVER_MAINTENANCE_ACTION_ROOT}" "${SERVER_MAINTENANCE_LIB}"
...
chown root:root "${SERVER_MAINTENANCE_ROOT}/running"
...
chmod 0700 "${SERVER_MAINTENANCE_ROOT}/running"
