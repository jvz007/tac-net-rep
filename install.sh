...
VERIFY_CONTRACT_CODE="from tec_tac.contracts import build_contract_catalog,render_markdown,render_text; c=build_contract_catalog(); expected='${PACKAGE_VERSION}'; assert c['framework_version']==expected, {'expected': expected, 'actual': c['framework_version']}; assert any(x['name']=='get_capability' for x in c['core']); assert any(x['name']=='revoke_user_sessions' and x['area']=='session-security' for x in c['core']); assert any(x['route']=='/api/tfd/contracts/' for x in c['http']); assert any(x['route']=='/api/tfd/session/current/' for x in c['http']); assert '# Tec-Tac Public Contracts' in render_markdown(c); assert 'TEC-TAC PUBLIC CONTRACTS' in render_text(c); print('TEC-TAC developer contract catalog OK:', c['counts'], 'version='+expected)"
